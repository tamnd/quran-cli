...
describes what users get. Pick a format with `-o`, or let quran choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
quran <command> -o table   # aligned columns for reading
quran <command> -o jsonl   # one JSON object per line, for piping
quran <command> -o json    # a single JSON array
quran <command> -o csv     # spreadsheet friendly
quran <command> -o tsv     # tab-separated
quran <command> -o url     # just the URL column
quran <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
quran <command> --fields id,title,url
```
...
```bash
quran <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
quran <command>            # a table, because this is a terminal
quran <command> | wc -l    # JSONL, because this is a pipe
```
