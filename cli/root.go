// Package cli assembles the quran command tree from the quran
...
// NewApp assembles the kit application from the quran domain. The
...
//
// To add a command, declare it in quran/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := quran.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(quran.Domain{}).Register(app)
